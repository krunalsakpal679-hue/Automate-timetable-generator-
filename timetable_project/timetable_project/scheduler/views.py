...
def home(request):
    ...
    if request.method == "POST":
        ...
        if mapping_form.is_valid():
            group = mapping_form.cleaned_data.get("group")
            subject = mapping_form.cleaned_data.get("subject")
            ...
            if group and subject:
                ...
                messages.success(request, f"Mapping for {group} ↔ {subject} saved.")
                saved = True
            else:
        ...
        if saved:
            ...
            messages.info(request, "No valid data to save.")
    ...
    for group, days in structured.items():
        ...
        for i in range(1, settings.periods_per_day + 1):
            ...
            for d in DAY_NAMES:
                val = days[d].get(i)
                if val:
                    cell = f"{val.subject.name}<br/>{val.teacher.name}<br/>{val.room.name}"
                else:
                    cell = "-"
                row.append(cell)
            data.append(row)
